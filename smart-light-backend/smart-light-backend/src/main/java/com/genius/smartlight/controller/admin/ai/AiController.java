...
public class AiController {
    ...
    public CommonResult<FabricRecognizeRespVO> fabricRecognize(@RequestPart("file") MultipartFile file) {
        return CommonResult.success(aiService.fabricRecognize(file));
    }

    @PostMapping(value = "/person-detect", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public CommonResult<PersonDetectRespVO> personDetect(@RequestPart("file") MultipartFile file) {
        return CommonResult.success(aiService.personDetect(file));
    }
}